...
namespace
{
    ...
    // This will store the number of particles so it can be passed to the new type of System
    // when things are toggled with 't'
    int n_Particles = 0;

    // Variable for the cloth system
    bool isCloth = false;
    // Variable to determine which system we are using
    int systemToggle = 0;
  ...
  void initSystem(int argc, char * argv[])
  {
    ...
    // set time stepper given user input
    if(argc == 3)
    {
        string c = argv[1];
        ...
        cout << "Usage: ./assignment_3 e/t/r stepSize\n";

    // seed the random number generator with the current time
    srand( time( NULL ) );

    // start out with simple system
        ...
        switch (key)
{
    ...
    case 't': 
    {
        ...
        {
            // Case to display the SimpleSystem
            case 0:
                isCloth = false;
                system = new SimpleSystem();
                cout << "Toggle: SimpleSystem" << endl;
                break;
            
            // Case to display the SimplePendulum
            case 1:
                isCloth = false;
                system = new PendulumSystem(1);
                cout << "Toggle: PendulumSystem" << endl; 
                break;

            // Case to display the SimpleChain
            case 2:
                isCloth = false;
                system = new PendulumSystem(3);
                cout << "Toggle: SimpleChain" << endl;
                break;
            
            // Case to display the ClothSystem
            case 3:
                isCloth = true;
                system = new ClothSystem(2);
                cout << "Toggle: ClothSystem" << endl;
                break;
...
int main( int argc, char* argv[] )
{
    ...
    glutCreateWindow("Michael Clausen: Assignment 4");
